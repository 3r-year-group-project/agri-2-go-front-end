...
export default function OrderComponent(props) {
    ...
    const handleClickOpen2 = () => {
        ...
        axios.post('/api/wrc/wastage_orders/getOrderDetails', { email: user.email, sellerId: props.sellerId, orderId: props.orderId })
            .then(res => {
                ...
                setWasteOrderData(old=>{return { ...old, quality: res.data.data[0].quality} });
                setWasteOrderData(old=>{return { ...old, quantity: res.data.data[0].quantity }});
                setWasteOrderData(old=>{return { ...old, fullName: completeName }});
                setWasteOrderData(old => {return { ...old, address: address }});

                setWasteOrderData(old => {return{ ...old, price: res.data.data[0].price }});
    ...
    const onClickCompleted = async () => {
        await axios.post('api/wrc/wastage_orders', { operation: 'MarkCollected', orderId: props.orderId })
        const { data } = await axios.get('api/wrc/wastage_orders')
